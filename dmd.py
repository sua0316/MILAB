...
class DMD(nn.Module):
    ...
    def forward(self, text, audio, video, is_distill=False):
        ...
        #print('text.shape = ', text.shape, ', video.shape = ', video.shape, ', audio.shape = ', audio.shape)
        x_l = F.dropout(text.transpose(1, 2), p=self.text_dropout, training=self.training)
        x_a = audio.transpose(1, 2)
        x_v = video.transpose(1, 2)
        #print('x_l.shape = ', x_l.shape, ', x_v.shape = ', x_v.shape, ', x_a.shape = ', x_a.shape )

        proj_x_l = x_l if self.orig_d_l == self.d_l else self.proj_l(x_l)
        proj_x_a = x_a if self.orig_d_a == self.d_a else self.proj_a(x_a)
        proj_x_v = x_v if self.orig_d_v == self.d_v else self.proj_v(x_v)

        #print('proj_x_l.shape = ', proj_x_l.shape, ', proj_x_v.shape = ', proj_x_v.shape, ', proj_x_a.shape = ', proj_x_a.shape)
        ...
        #print('hs_l_low.shape = ', hs_l_low.shape)
        ...
        res = {
            ...
            'logits_c': logits_c,
            'output_logit': output,
            'last_hs': last_hs,  # last_hs 추가
            'last_hs_proj': last_hs_proj,  # last_hs_proj 추가
        }
        return res