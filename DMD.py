        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    #lamda = 0.5
                    #augmented_text = (text[most_similar_indices] - text) * 0.5 + text
                    #augmented_audio = (audio[most_similar_indices] - audio) * 0.5 + audio
                    #augmented_video = (vision[most_similar_indices] - vision) * 0.5 + vision
                    ...
                    #'''''
                    # 1. pertubation
                    per_flag = torch.rand(audio.shape[0]) # 0~1

                    #text_noise = torch.randn_like(text) * noise_std
                    text_noise = torch.rand_like(origin_l) * 2 - 1 # [-1,1]
                    augmented_text = origin_l + text_noise
                    for xx in range(origin_l.shape[0]):
                        if per_flag[xx] == 0:
                            augmented_text[xx, :, :] = origin_l[xx, :, :] + text_noise[xx, :, :]
                        else:
                            augmented_text[xx, :, :] = origin_l[xx, :, :] * text_noise[xx, :, :]
                    
                    #audio_noise = torch.randn_like(audio) * noise_std
                    audio_noise = torch.rand_like(audio) * 2 - 1 # [-1,1]
                    augmented_audio = audio + audio_noise
                    for xx in range(audio.shape[0]):
                        if per_flag[xx] == 0:
                            augmented_audio[xx,:,:] = audio[xx,:,:] + audio_noise[xx,:,:]
                        else:
                            augmented_audio[xx,:,:] = audio[xx,:,:] * audio_noise[xx,:,:]

                    #video_noise = torch.randn_like(vision) * noise_std
                    video_noise = torch.rand_like(vision) * 2 - 1
                    augmented_video = vision + video_noise
                    for xx in range(audio.shape[0]):
                        if per_flag[xx] == 0:
                            augmented_video[xx,:,:] = vision[xx,:,:] + video_noise[xx,:,:]
                        else:
                            augmented_video[xx,:,:] = vision[xx,:,:] * video_noise[xx,:,:]
                    #'''''
                    ...
                    #loss_task = augmented_output_loss_task
